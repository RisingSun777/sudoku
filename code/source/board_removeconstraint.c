...
void board_removeconstraint(boardptr b, int row, int col, int value) {
    ...
    for (int i = 0; i < MAXSIZE; ++i)
        b[row][i].constraints = SETBIT(value, b[row][i].constraints);

    /* update constraint for col */
    for (int i = 0; i < MAXSIZE; ++i)
        b[i][col].constraints = SETBIT(value, b[i][col].constraints);
    ...
    for (int i = minrow; i <= maxrow; ++i)
        for (int j = mincol ; j <= maxcol; ++j)
            b[i][j].constraints = SETBIT(value, b[i][j].constraints);
}
